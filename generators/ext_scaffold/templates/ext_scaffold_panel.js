<% if controller_class_nesting_depth > 0 %>
	<%= "Ext.namespace('ExtScaffold.#{controller_class_nesting.gsub(/::/,'.')}');" %>
<% end %>
...
<%= @attributes.inject('') {|labels, a| labels << "    ,'#{class_name.demodulize.underscore}[#{a.name}]': '#{a.name.humanize}'\n" } -%>
  ...
  printtButtonTooltip: 'Print Listing of <%= class_name.demodulize %>s',
  exportButtonTooltip: 'Export to Excel',
  ...
  workbookTitle: '<%= class_name.demodulize %> Listing',
  //
  // custom properties
  //
  url: '#',
...
<%= @attributes.inject('') {|columns, a| columns << "      ,{ header: scaffoldPanel.labels['#{class_name.demodulize.underscore}[#{a.name}]'], dataIndex: '#{class_name.demodulize.underscore}[#{a.name}]', sortable: true }\n" } -%>
    ]);
    ...
    function printButtonHandler(){
	 	 var grid = scaffoldPanel.getGridPanel() ;
		 Ext.ux.Printer.BaseRenderer.prototype.stylesheetPath = '/stylesheets/print.css';
...
<%= @attributes.inject([]) do |items, a|	
     ...
     item << case a.field_type
       when :number_field    then  ", xtype: 'numberfield'"
...
%>
  ...
  onRender: function() {
	...
	//Added for Export to Excel Button
	var gp = this.getGridPanel();
	/*var expButton = new Ext.ux.Exporter.Button({
		 	component: gp,
			text:  this.exportButtonLabel,
			tooltip: this.exportButtonTooltip,
			title: this.workbookTitle
		 });  
  	gp.getTopToolbar().insertButton(4,expButton);	
	*/
